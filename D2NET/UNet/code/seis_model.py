...
from UNet.code.utils import sinc_interp_factor, plotseis
...
class SeisModel:
    ...
    min_thickness: int
    max_thickness: int
    interp_signal: Optional[Union[int, List[int]]]
    filename_signal: str

    path_signals: List[Path]
    ...
    def __init__(self, smoothness: int = 5,
                 ...
                 interp_signal: Optional[Union[int, List[int]]] = None,
                 filename_signal: str = ''):
    ...
    def get_random_model(self) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _generate_model(self) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _create_surface_wave(self, x0: np.ndarray, width: np.ndarray) -> np.ndarray:
    ...
    def _prepare_path(self) -> List[Path]:
        path = Path('UNet/code/signals').resolve()
    ...
    def _get_signal(self) -> np.ndarray:
        ...
        if self.interp_signal is None or self.interp_signal == 1:
            return signal
        elif isinstance(self.interp_signal, (tuple, list)):
            return sinc_interp_factor(signal, np.random.randint(self.interp_signal[0], self.interp_signal[1]))
            ...
            return sinc_interp_factor(signal, np.random.randint(1, 7))
        else:
            err_interp_str = '"interp_signal" must be integer, list with integer or -1'
    ...
    def _get_signal_filename(self) -> Path:
    ...
    def _limit_size_model(self, raw_model: np.ndarray) -> np.ndarray:
...
if __name__ == '__main__':
    ...
    plotseis(seismic_gather, picking_gather, colorseis=True)
